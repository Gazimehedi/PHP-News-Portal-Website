        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <div class="post-container">
                  ...
                  $offset = ($page - 1) * $limit;
                  $cquery = "SELECT post,category_name FROM category WHERE category_id = {$cid}";
                  $cresult = mysqli_query($connect,$cquery);
                  $category = mysqli_fetch_assoc($cresult);
                  echo "<h2 class='page-heading'>{$category['category_name']}</h2>";
                  ...
                  if(mysqli_num_rows($runquery) > 0){
                    ...
                    $query1 = "SELECT * FROM category WHERE category_id = {$cid}";
                    $runquery1 = mysqli_query($connect,$query1) or die("Query Failed".mysqli_error());
                    if(mysqli_num_rows($runquery1) > 0){
                      $data = mysqli_fetch_assoc($runquery1);
                      $total_data = $data['post'];
